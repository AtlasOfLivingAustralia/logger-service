...
BEGIN
	...
	SELECT COUNT(*) FROM event_summary_breakdown_email_entity esbee 
	 ...
	 IF count_summary_total_rows = 0 THEN
        ...
        VALUES (new_month, new_log_event_type_id, 1, NEW.record_count);
	 ELSE
		-- only update the event count once per event!
        IF count_total_detail_rows_for_event = 1 then
	 ...
	 IF count_breakdown_reason_rows = 0 THEN
        ...
        VALUES (new_month, new_log_event_type_id, new_log_reason_type_id, 1, NEW.record_count);
	 ELSE
		-- only update the event count once per event!
        IF count_total_detail_rows_for_event = 1 then
			UPDATE event_summary_breakdown_reason esrb SET number_of_events = number_of_events + 1, record_count = record_count + NEW.record_count
	 ...
	 IF count_breakdown_email_rows = 0 THEN
        ...
        VALUES (new_month, new_log_event_type_id, new_user_email_category, 1, NEW.record_count);
	 ELSE
		-- only update the event count once per event!
        IF count_total_detail_rows_for_event = 1 then
			UPDATE event_summary_breakdown_email esbe SET number_of_events = number_of_events + 1, record_count = record_count + NEW.record_count
